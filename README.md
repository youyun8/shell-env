...
Configures Bash with a managed environment block and a Git Bash-style two-line prompt (a blank line, green `user@host`, yellow full path, cyan Git status, then the command on a new line), plus terminal-default command input and Vim settings.
...
Installs [fish](https://fishshell.com/) and can write managed environment, prompt, color, and Vim configuration blocks. The prompt mirrors the Git Bash style: a blank line, green `user@host`, yellow full path, and cyan Git status, with the command on a new line; editable command syntax uses the terminal default to match Bash.
...
Installs [Nushell](https://www.nushell.sh/) and can write managed environment, prompt, and Vim configuration. The prompt mirrors the Git Bash style: a blank line, green `user@host`, yellow full path, and cyan Git status, with the command on a new line; editable command syntax uses the terminal default to match Bash.
